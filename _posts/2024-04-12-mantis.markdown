...
Similar to visual instruction tuning[^4], data curation is an important step to ensure good performance. During the curation, we have found some heuristics that can improve the performance. For example, we reformat some classification tasks into multiple-choice QA format to avoid conflicts between short answers and long answers. We also add image denotations in the text for each question to make sure the question involved with multiple images is clear to answer. Besides, We write some rules to randomly put the image placeholders at the beginning of the first question or the end of the first question for each item with multiple images. This technique is also used by the curation LLaVA-665k[^5].
...
### Baselines
For the Mantis-Instruct-eval evaluation, we have included some famous LMMs like BLIP-2, InstructBlip, Qwen-VL, LLaVA, Kosmos2, and Fuyu. Since these models cannot accept multiple images as input, we deliberately merge the images into a single one, putting them side by side so that they can serve as the baselines.
...
*Table 2: The performance of Mantis-llava-7b-v1.0 and Mantis-llava-7b-v1.1 on NLVR2, Birds-to-words, and Mantis-Instruct-eval evaluation.*

We report results on birds-to-words, NLVR2, and our curated human-eval benchmark. As shown in the table, our model achieves decent performance on the benchmarks. Specifically, Mantis-llava-7b-v1.0 gets 84.93 on NLVR2 and 52.52 accuracies on Birds-to-words. Mantis-llava-7b-v1.1 gets 50.69 on the Human-eval subset, which surpasses the LLaVA-1.5 by 19.35 and BLIP2 by 0.92.
...
*Table 3: The performance of Mantis-llava-7b-v1.0 and Mantis-llava-7b-v1.1 on Qbench2-A1-single-dev evaluation.*
...
![Table 4: The performance of Mantis-llava-7b-v1.1 on Mementos behavior recognition evaluation.]({{"/assets/Mantis/images/mementos.jpeg" | relative_url }})
